...
function UserService(ls, log) {
  ...
  function getUserById(data, cb) {
    ...
    mongoDBService.findOne("tambolaDB", "users", query, {}, function (
      error,
      result
    ) {
      if (error) {
  ...
  function getVersion(data, cb) {
    mongoDBService.findOne("luckyDrawDB", "gameVersion", {}, {}, function (
      error,
      result
    ) {
      if (error) {
  ...
  function updateVersion(data, cb) {
    var query = { game: data.game };
    mongoDBService.update("luckyDrawDB", "gameVersion", query,{ $set: data }, { upsert: true },function (error, result) {
        if (error) {
          cb(error, null);
          return;
        }
